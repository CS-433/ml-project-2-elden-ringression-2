...
class Feeder_kinetics(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for frame_info in video_info['annotations']:
            ...
            for m, skeleton_info in enumerate(frame_info["keypoints"]):
                ...
                data_numpy[0, frame_index, m, 0] = skeleton_info[0]
                data_numpy[1, frame_index, m, 0] = skeleton_info[1]
                data_numpy[2, frame_index, m, 0] = skeleton_info[2]
            mean = (data_numpy[:, frame_index, 5, 0]+data_numpy[:, frame_index, 6, 0]+data_numpy[:, frame_index, 11, 0]+data_numpy[:, frame_index, 12, 0])/4
            data_numpy[:, frame_index, :, 0] = data_numpy[:, frame_index, :, 0] - mean.reshape(3,1)
        # centralization
        ...
        if self.data_path.split('.')[-1][-1] == 't':#if test set
            label = video_info['category_id']
        else:
            label = video_info['category_id']%4
        # label = self.label[index]
        # print(label,self.label[index])
        # assert (self.label[index] == video_info['category_id']%4)
